...
const SeatChart = ({
  ...
  const buyHandler = async (_seat) => {
    ...
    if (discountEligible > 0) {
      //minting with discount

      const ticketCostDetails = await tokenMaster.getOccasion(occasion.id);
      const ticketCostInEth = ethers.BigNumber.from(ticketCostDetails.cost);
      const discount = ticketCostInEth.mul(10).div(100);
      const discountedCost = ticketCostInEth.sub(discount);
      console.log(discountedCost, "discounted cost final");
    } else {
      // Normal minting without discount
      const transaction = await tokenMaster
        .connect(signer)
        .mint(occasion.id, _seat, { value: occasion.cost });
      await transaction.wait();
